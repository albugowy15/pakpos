...
On x86_64 Linux, install the latest stable release into `~/.local/bin` with:
...
Set `PAKPOS_INSTALL_DIR` to choose another destination:

```sh
curl --proto '=https' --tlsv1.2 -sSf \
  https://raw.githubusercontent.com/albugowy15/pakpos/main/install.sh \
  | PAKPOS_INSTALL_DIR=/usr/local/bin sh
```
