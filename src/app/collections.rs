...
impl CollectionSession {
    ...
    pub fn from_tree(summary: CollectionSummary, nodes: Vec<CollectionNode>) -> Self {
        let mut folders = Vec::new();
        let mut requests = Vec::new();
        for node in nodes {
            match node.kind {
                CollectionNodeKind::Folder => folders.push(node),
                CollectionNodeKind::Request => requests.push(SessionRequest {
    ...
    pub fn request_source(&self, request_id: Uuid) -> Option<(CollectionNode, Option<Request>)> {
        self.requests
            .iter()
            .find(|request| request.node.id == request_id)
    ...
    pub fn request_items(&self) -> Vec<RequestListItem> {
        self.requests
            .iter()
            .map(|request| RequestListItem {
                id: request.node.id,
                name: request.node.name.clone(),
    ...
    pub fn duplicate_request(&mut self, source_node: CollectionNode, request: Request) -> Uuid {
...
mod tests {
    ...
    fn saved_session() -> (CollectionSession, Uuid) {
        ...
        let saved = CollectionRequest::new(summary.id, None, "List users", 0, Request::default());
        let id = saved.node.id;
        let mut session = CollectionSession::from_tree(summary, vec![saved.node.clone()]);
    ...
    fn loaded_session_starts_clean_and_request_bodies_stay_lazy() {
        let summary = CollectionSummary::new("API");
        let node = CollectionNode::request(summary.id, None, "List users", 0);
        let id = node.id;
        let session = CollectionSession::from_tree(summary, vec![node]);
